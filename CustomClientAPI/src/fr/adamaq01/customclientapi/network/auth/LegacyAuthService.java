...
public class LegacyAuthService implements AuthService<LegacySession> {
	...
	@Override
	public LegacySession login(String username, String password, ProxyData proxy)
			throws AuthenticationException, IOException {
		String parameters = String.format(LOGIN_URL_PARAMETERS, encodeUtf8(username), encodeUtf8(password),
				LOGIN_VERSION);
	...
	public void authenticate(LegacySession session, String serverId, ProxyData proxy)
			throws AuthenticationException, IOException {
		Proxy wrappedProxy = wrapProxy(proxy);
		URL url = new URL(String.format(SERVER_AUTH_URL, encodeUtf8(session.getUsername()),
				encodeUtf8(session.getSessionId()), encodeUtf8(serverId)));
